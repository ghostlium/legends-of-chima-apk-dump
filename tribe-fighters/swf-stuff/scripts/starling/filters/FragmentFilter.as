package starling.filters
{
   ...
   public class FragmentFilter
   {
      ...
      private function renderPasses(param1:DisplayObject, param2:RenderSupport, param3:Number, param4:Boolean = false) : QuadBatch
      {
         ...
         this.calculateBounds(param1,_loc8_,this.mResolution * _loc10_,!param4,_loc13_,_loc14_);
         if(_loc13_.isEmpty())
         {
            this.disposePassTextures();
         ...
         var _loc15_:Texture;
         if((_loc15_ = param2.renderTarget) && !SystemUtil.supportsRelaxedTargetClearRequirement)
         {
            throw new IllegalOperationError("To nest filters, you need at least Flash Player / AIR version 15.");
         }
         if(param4)
         {
            _loc6_ = Texture.empty(_loc14_.width,_loc14_.height,this.PMA,false,true,this.mResolution * _loc10_);
         }
         ...
         var _loc16_:int = 0;
         while(_loc16_ < this.mNumPasses)
         {
            if(_loc16_ < this.mNumPasses - 1)
            {
               param2.renderTarget = this.getPassTexture(_loc16_ + 1);
               param2.clear();
            }
            else if(param4)
            {
            ...
            this.activate(_loc16_,_loc7_,_loc5_);
            _loc7_.drawTriangles(this.mIndexBuffer,0,2);
            this.deactivate(_loc16_,_loc7_,_loc5_);
            _loc16_++;
         }
         ...
         if(param4)
         {
            ...
            _loc17_.addImage(_loc18_,1,sTransformationMatrix);
            return _loc17_;
         }
         return null;
      }
      ...
      private function updatePassTextures(param1:Number, param2:Number, param3:Number) : void
      {
         ...
         var _loc5_:Boolean;
         if(_loc5_ = this.mPassTextures.length != _loc4_ || Math.abs(this.mPassTextures[0].nativeWidth - param1 * param3) > 0.1 || Math.abs(this.mPassTextures[0].nativeHeight - param2 * param3) > 0.1)
         {
            this.disposePassTextures();
            _loc6_ = 0;
            while(_loc6_ < _loc4_)
            {
               this.mPassTextures[_loc6_] = Texture.empty(param1,param2,this.PMA,false,true,param3);
               _loc6_++;
            }
         }
      }
      ...
      private function calculateBounds(param1:DisplayObject, param2:DisplayObject, param3:Number, param4:Boolean, param5:Rectangle, param6:Rectangle) : void
      {
         ...
         if(!param5.isEmpty())
         {
            ...
            _loc11_ = param5.width > _loc10_ ? Number(param5.width) : Number(_loc10_);
            _loc12_ = param5.height > _loc10_ ? Number(param5.height) : Number(_loc10_);
