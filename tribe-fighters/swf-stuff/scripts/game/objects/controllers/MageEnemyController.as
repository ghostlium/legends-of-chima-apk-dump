package game.objects.controllers
{
   ...
   public class MageEnemyController extends EnemyController
   {
      
      public static const kfTestEnemyMoveSpeed:Number = 150;
      ...
      public static const kfEnemyCloseDistance:Number = 30;
      ...
      public function MageEnemyController(param1:Enemy)
      {
         super(param1);
      ...
      public function StateWalk(param1:int, param2:int) : uint
      {
         ...
         {
            case StateManager.Z_ACTION_START:
               this.pickNewWalkTarget(new Vec2F(0,0),new Vec2F(LegoChima.Instance().zGetScreenWidth(),LegoChima.Instance().zGetScreenHeight()));
            ...
            case StateManager.Z_ACTION_UPDATE:
               if(this._pAnims[_pEnemy.getCurrAnim()].isAnimating() == false)
               {
                  ...
                  {
                     _loc5_ = false;
                     if(_loc4_ && Math.abs(_loc4_.getPosition().x - _pEnemy.getPosition().x) <= kfEnemyCloseDistance)
                     {
                        _loc5_ = true;
                     }
                     if(_loc5_ && !_loc3_)
      ...
      public function pickNewWalkTarget(param1:Vec2F, param2:Vec2F) : void
      {
         ...
         var _loc5_:Player = GameWorld.get().getPlayer();
         if(Random.RandomNumberRange(0,10000) % 2 == 0 && _loc5_ && _pEnemy.getPosition().y < _loc5_.getPosition().y)
         {
            this._vWalkTarget.x = _loc5_.getPosition().x;
         }
         this._vVelocity.x = this._vWalkTarget.x - _pEnemy.getPosition().x;
         this._vVelocity.y = this._vWalkTarget.y - _pEnemy.getPosition().y;
         this._vVelocity.normalize(1);
