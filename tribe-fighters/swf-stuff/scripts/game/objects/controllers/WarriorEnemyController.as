package game.objects.controllers
{
   ...
   public class WarriorEnemyController extends EnemyController
   {
      
      public static const kfTestEnemyMoveSpeed:Number = 200;
      ...
      public static const kfEnemyMaxXMovement:Number = 100;
      ...
      public function WarriorEnemyController(param1:Enemy)
      {
         super(param1);
      ...
      public function StateWalk(param1:int, param2:int) : uint
      {
         ...
         {
            case StateManager.Z_ACTION_START:
               if(_pEnemy.getClassType() == GameConsts.eChimaWarriorClass)
               {
            ...
            case StateManager.Z_ACTION_UPDATE:
               ...
               {
                  _loc3_ = false;
                  if((_loc4_ = GameWorld.get().getPlayer()) && _pEnemy.getPosition().y >= kfPlayerBaseLineY)
                  {
                     _loc3_ = true;
                  }
                  ...
                  else if(Random.RandomNumberRange(0,10000) % 2 == 0 || _loc3_)
                  {
                     this.setEnemyAnimation(ChimaGameObject.CHAR_ANIM_RUN);
                  }
                  else
      ...
      public function pickNewWalkTarget() : void
      {
         var _loc1_:Vec2F = new Vec2F(this._vWalkTarget.x,this._vWalkTarget.y);
         var _loc2_:Vec2F = new Vec2F(_loc1_.x - this._vWalkTarget.x,_loc1_.y - this._vWalkTarget.y);
         while(_loc2_.getLengthSq() < 300)
         {
            this._vWalkTarget.x = _pEnemy.getPosition().x + Random.RandomFloatRangeUniform(-1 * kfEnemyMaxXMovement,kfEnemyMaxXMovement);
            this._vWalkTarget.x = MathsHelper.Clamp(this._vWalkTarget.x,0,LegoChima.Instance().zGetScreenWidth());
            this._vWalkTarget.y = Random.RandomFloatRangeUniform(_pEnemy.getPosition().y,_pEnemy.getPosition().y + 0.25 * Number(LegoChima.Instance().zGetScreenHeight()) + Random.RandomFloatRangeUniform(0,0.5) * Number(LegoChima.Instance().zGetScreenHeight()));
            _loc2_.x = _loc1_.x - this._vWalkTarget.x;
            _loc2_.y = _loc1_.y - this._vWalkTarget.y;
         }
         this._vVelocity.x = this._vWalkTarget.x - _pEnemy.getPosition().x;
         this._vVelocity.y = this._vWalkTarget.y - _pEnemy.getPosition().y;
         this._vVelocity.normalize(1);
